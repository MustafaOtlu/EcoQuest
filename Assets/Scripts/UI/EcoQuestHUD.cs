...
public sealed class EcoQuestHUD : MonoBehaviour
{
    ...
    private Text[] slotCounts, barLabels;
    private Text mode, inventory, context, feedback, instructions, dead;
    ...
    private void Start()
    {
        ...
        var left = Panel("Vitals", root.transform, new Vector2(0,1), new Vector2(24,-24), new Vector2(330,156), new Vector2(0,1));
        Label(left, "ECOQUEST", new Vector2(14,-10), new Vector2(300,28), 21, TextAnchor.MiddleLeft);
        fills = new Image[3]; barLabels = new Text[3];
        string[] labels = { "CAN", "ENERJİ", "SU" };
        Color[] colors = { new Color(0.85f,0.32f,0.3f), new Color(0.8f,0.76f,0.24f), new Color(0.23f,0.65f,0.85f) };
        for(int i=0;i<3;i++)
        {
        ...
        var bag = Panel("Materials",root.transform,new Vector2(1,1),new Vector2(-24,-24),new Vector2(300,156),new Vector2(1,1));
        Label(bag,"KAYNAKLAR",new Vector2(14,-10),new Vector2(274,28),20,TextAnchor.MiddleLeft);
        inventory=Label(bag,"",new Vector2(14,-43),new Vector2(274,100),18,TextAnchor.UpperLeft);
        var dock=Panel("Toolbelt",root.transform,new Vector2(0.5f,0),new Vector2(0,24),new Vector2(824,112),new Vector2(0.5f,0));
        slots=new Image[5];slotRects=new RectTransform[5];slotCounts=new Text[5];
        for(int i=0;i<5;i++)
        {
            ...
            Label(slot,names[i],new Vector2(6,-31),new Vector2(140,24),i==2?14:17,TextAnchor.MiddleCenter);
        ...
        var note=Panel("Feedback",root.transform,new Vector2(0.5f,1),new Vector2(0,-34),new Vector2(660,55),new Vector2(0.5f,1));
        ...
        var death=Panel("Respawn",root.transform,new Vector2(0.5f,0.5f),new Vector2(0,60),new Vector2(500,50),new Vector2(0.5f,0.5f));
    ...
    private void Update()
    {
        ...
        for(int i=0;i<5;i++)
        {
            bool selected=(int)weapons.Selected==i;
    ...
    private void Refresh()
    {
        ...
        float[] values={vitals!=null?vitals.Health:100,weapons.Energy,weapons.Water};
        float[] maxima={vitals!=null?vitals.maximumHealth:100,weapons.maximumEnergy,weapons.maximumWater};
        string[] labels={"CAN","ENERJİ","SU"};
        for(int i=0;i<3;i++)
        ...
        feedback.text=Time.time<weapons.FeedbackUntil?weapons.Feedback:"";
        dead.text=vitals!=null&&vitals.IsDead?"Yeniden doğuyorsun…":"";
    }
    ...
    private Text Label(Transform parent,string content,Vector2 position,Vector2 size,int fontSize,TextAnchor align)
    {
        ...
        var text=go.GetComponent<Text>();text.font=font;text.fontSize=Mathf.Max(18,fontSize)*rasterScale;text.color=cream;text.alignment=align;text.text=content;text.raycastTarget=false;
